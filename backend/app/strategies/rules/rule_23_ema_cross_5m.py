...
def _evaluate(data: MarketData) -> RuleResult:
    ...
    fast = ema(closes, 5)
    slow = ema(closes, 9)
    offset = len(closes) - len(slow) if len(slow) > 0 else 0
    ...
    if last_cross == "golden" and bars_ago == 0:
        return RuleResult(score=2, side="long", detail=f"EMA5 crossed above EMA9 on current 5m candle (offset={offset})")
    if last_cross == "death" and bars_ago == 0:
        return RuleResult(score=-2, side="short", detail="EMA5 crossed below EMA9 on current 5m candle")
    if last_cross == "golden" and bars_ago > 0 and bars_ago <= 3:
        return RuleResult(score=1, side="long", detail=f"EMA5/9 golden cross {bars_ago} bars ago (5m)")
    if last_cross == "death" and bars_ago > 0 and bars_ago <= 3:
        return RuleResult(score=-1, side="short", detail=f"EMA5/9 death cross {bars_ago} bars ago (5m)")
    if aligned == "up":
        return RuleResult(score=0.5, side="long", detail="EMA5 above EMA9 (5m uptrend)")
    if aligned == "down":
        return RuleResult(score=-0.5, side="short", detail="EMA5 below EMA9 (5m downtrend)")
    return RuleResult(score=0, side="neutral", detail="EMA5/9 flat (5m)")


rule_23_ema_cross_5m = TradingRule(
    key="rule_23_ema_cross_5m",
    name="EMA 5/9 Crossover (5m, TP2/SL1)",
